...
int64_t GetProofOfWorkReward(int64_t nFees)
{
     ...
     else if (pindexBest->nHeight+1 <= 50000)
    {
     ...
     else if (pindexBest->nHeight+1 <= 60000)
    {
       nSubsidy = 0.5 * COIN;
        return nSubsidy + nFees;
    }
    
     else if (pindexBest->nHeight+1 <= 70000)
    {
      nSubsidy = 0.4 * COIN;
      return nSubsidy + nFees;
    }
	...
	else if (pindexBest->nHeight+1 <= 90000)
    {
      nSubsidy = 0.2 * COIN;
      return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SUMCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "SUMCoin LAUNCH";
        CTransaction txNew;
        txNew.nTime = 1402700130;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402700132;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3403945;
		if(fTestNet)
        {
            block.nNonce   = 3403945;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x933ae44a976afffbfb02908144602fbafcca86c7a358236aa40174d1356cfb73"));
        block.print();
